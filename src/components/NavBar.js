...
export default function NavBar() {
  ...
  const addressWallet = useSelector((state) => state.signerReducer.address);
  ...
  async function handleConnection() {
    ...
    try {
      ...
      dispatch(updateAddress(key.bech32Address));
    ...
    <div id="navbar" className="bg-white sticky top-0 z-50">
      ...
      >
        <div className="flex lg:flex-1">
          ...
          <span class="ml-2 p-1 text-md text-opacity-90 font-bold">
            3M1FCoin
          </span>
        </div>
        ...
        <div className="hidden -mt-3 lg:flex lg:flex-1 lg:justify-end">
          {connected ? (
            ...
            <button
              className="bg-blue-200 hover:bg-blue-300 text-blue-600 font-bold py-2 px-4 rounded-full flex items-center"
      ...
      <hr class="h-px -mt-2 border-t-0 bg-transparent bg-gradient-to-r from-transparent via-neutral-700 to-transparent opacity-25 dark:via-neutral-400" />
    </div>
  );
}
